package com.umomo.Stock.ui.home
...
import com.umomo.Stock.R
import com.umomo.Stock.setting.AppDataInstance
import com.umomo.Stock.setting.GPSTrack
...
class HomeActivity : ThemeBaseActivity(), NavigationView.OnNavigationItemSelectedListener{
    ...
    private fun initView(){
        ...
        //pull_to_refresh.isRefreshing = PreferenceUtils.getInstance().getBooleanValue(Constants.KEY_PULL_TO_REFRESH_ENABLE, false)

        // set loader style
        val secret: SecretKey?
        val toEncrypt = PreferenceUtils.getInstance().getStringValue(Constants.KEY_WEB_URL, defaultURL)
        try {
            secret = generateKey()
            val toDecrypt = encryptMsg(toEncrypt!!, secret)

            UtilMethods.printLog(TAG, toDecrypt.toString())
            UtilMethods.printLog(TAG, decryptMsg(toDecrypt, secret))
        } catch (e: Exception) {
            UtilMethods.printLog(TAG, "" + e.message)
        }
    ...
    private fun initThemeStyle(){
        // loader style
        val loaderStyle = PreferenceUtils.getInstance().getStringValue(Constants.KEY_LOADER, Constants.LOADER_CIRCLE)
    ...
    private fun initWebClient(){
        webViewClient = object : WebViewClient() {
            ...
            override fun onReceivedError(view: WebView?, request: WebResourceRequest?, error: WebResourceError?) {
                ...
                //UtilMethods.showShortToast(mContext, error.toString())
            ...
            override fun onReceivedHttpError(view: WebView?, request: WebResourceRequest?, errorResponse: WebResourceResponse?) {
                ...
                //UtilMethods.showShortToast(mContext, "HTTP Error: ${errorResponse.toString()}")
            }

            override fun onReceivedSslError(view: WebView?, handler: SslErrorHandler?, error: SslError?) {
                super.onReceivedSslError(view, handler, error)
                UtilMethods.printLog(TAG, "SSL Error: ${error.toString()}")
                //UtilMethods.showShortToast(mContext, "SSL Error: ${error.toString()}")
            }
                    ...
                    if (PreferenceUtils.getInstance().getBooleanValue(Constants.KEY_FILE_UPLOAD_ENABLE, true)) {
                        ...
                        if (PreferenceUtils.getInstance().getBooleanValue(Constants.KEY_PHOTO_UPLOAD_ENABLE, true)) {
                            ...
                            if (takePictureIntent.resolveActivity(this@HomeActivity.packageManager) != null) {
                                ...
                                if (photoFile != null) {
                                    mFileCamMessage = "file:" + photoFile.getAbsolutePath()
    ...
    @Throws(NoSuchAlgorithmException::class, NoSuchPaddingException::class, InvalidKeyException::class,
            InvalidParameterSpecException::class, IllegalBlockSizeException::class, BadPaddingException::class,
    ...
    @Throws(NoSuchPaddingException::class, NoSuchAlgorithmException::class, InvalidParameterSpecException::class,
            InvalidAlgorithmParameterException::class, InvalidKeyException::class, BadPaddingException::class,
            IllegalBlockSizeException::class, UnsupportedEncodingException::class)
    ...
    private fun initSliderMenu() {
        ...
        //subMenu.add(1, R.string.menu_theme, Menu.NONE, getString(R.string.menu_theme)).setIcon(R.drawable.ic_select_theme)
        ...
        for(menu in AppDataInstance.navigationMenus){
            when(menu.url) {
                ...
                "SHARE" -> //subMenu = menu.addSubMenu(getString(R.string.menu_share));
                    subMenu.add(i++, R.string.menu_share, Menu.NONE, getString(R.string.menu_share)).setIcon(R.drawable.ic_share)
                else -> subMenu.add(i++, i, Menu.NONE, menu.menu).setIcon(R.drawable.ic_label)
            }
        }
    }
